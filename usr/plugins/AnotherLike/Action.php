...
class AnotherLike_Action extends Typecho_Widget implements Widget_Interface_Do
{
    ...
    public function up(){
        ...
        if($cid){
            try {
                $row = $this->db->fetchRow($this->db->select('likes')->from('typecho_contents')->where('cid = ?', $cid));
                $this->db->query($this->db->update('typecho_contents')->rows(array('likes' => (int)$row['likes']+1))->where('cid = ?', $cid));

...
class AnotherLike_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function activate()
    {
        ...
        if (!array_key_exists('likes', $db->fetchRow($db->select()->from('typecho_contents'))))
    ...
    public static function theLike($link = true){
        ...
        $row = $db->fetchRow($db->select('likes')->from('typecho_contents')->where('cid = ?', $cid));
        if($link){
    ...
    public static function theMostLiked($limit = 10, $showlink = true, $before = ' 赞: ', $after = ' 次')
    {
        ...
        $posts = $db->fetchAll($db->select()->from('typecho_contents')

...
data-toggle="tooltip" data-placement="right" data-original-title="' . _mt("阅读模式") . '"><i data-feather="book-open"></i></a>';
    ...
    public static function exportHtmlTag($options)
    {
        ...
        if (@in_array("opacityMode", Utils::checkArray($options))) {
...
EOF;
        ...
        if (Utils::getExpertValue('no_index_post_auto_array',false)){
            $css .= <<<EOF
.blog-post.post-list{
    display:block;
}
...
//        print_r($obj);
        while ($obj->next()) {
            ...
            if ($styleThumb == "DEFAULT" || trim($styleThumb) == "") {//跟随外观设置的配置
                ...
                } else if ($options->thumbStyle == "2" || $options->thumbStyle == "4") {//交错显示
                    if ($index % 2 == 0) {
                        if ($options->thumbStyle == "2"){
            ...
            if ($parameterArray['imgSrc'] == ""){
                if ($parameterArray["nothumbStyle"] != ""){
                    $parameterArray['thumbStyle'] = "NO_PIC_WITH_ICON";
                }else{
                    $parameterArray['thumbStyle'] = "NO_PIC";
                }
            }
...
//            print_r(($endTime-$startTime)*1000 . ' ms');
            ...
            if (in_array('NoSummary-index', Utils::checkArray($options->indexsetup))) {
                ...
                } else if($parameterArray['thumbStyle'] == "LARGE" || $parameterArray['thumbStyle'] == "PICTURE"){
                    if ($options->numberOfBigPic == "") {//自定义摘要字数
                        $expertNum = 200;
                    } else {
                        $expertNum = $options->numberOfBigPic;
                    }
                } else if ($parameterArray['thumbStyle'] == "NO_PIC"){
                    if ($options->numberOfNonePic){
                        $expertNum = $options->numberOfNonePic;
                    }else{
                        $expertNum = 80;
                    }
                }else if ($parameterArray['thumbStyle'] == "NO_PIC_WITH_ICON"){
                    if ($options->numberOfNonePicWithIcon){
                        $expertNum = $options->numberOfNonePicWithIcon;
                    }else{
                        $expertNum = 80;
                    }
                }
...
//                print_r(($endTime-$startTime)*1000 . ' ms');
            ...
            $parameterArray['isIndexFooter'] = !trim(@$obj->fields->pic_index_footer) == "false";;
...
//            $viewNumString = $parameterArray['viewNum'] . " " . _mt("次浏览");
        ...
        if ($parameterArray['isIndex']) {//首页界面的文章item结构
            //头图部分
            if ($parameterArray['thumbStyle'] == "NO_PIC") {//图片地址为空即不显示头图
                $html .= '<div class="single-post panel panel-no-pic box-shadow-wrap-normal">';
            }else if ($parameterArray['thumbStyle'] == "NO_PIC_WITH_ICON"){
                $html .= '<div class="single-post panel panel-no-pic with-icon box-shadow-wrap-normal">';
            } else {
                if ($parameterArray['thumbStyle'] == "LARGE") {//大版式头图
//                    $img = Utils::returnImageLazyLoadHtml(false,$parameterArray['imgSrc'], 800, 0);
...
EOF;
                    ...
                    $img = Utils::returnImageLazyLoadHtml(false,$parameterArray['imgSrc'], 1200, 0);
                    //TODO: picture
                    $html .= <<<EOF
<div class="single-post panel-picture border-radius-6 box-shadow-wrap-normal">
                <figure class="post-thumbnail border-radius-6">
                    <a class="post-thumbnail-inner index-image lazy" href="{$parameterArray['linkUrl']}"> 
                     <img $img class="lazy post-pic border-radius-6">
                     </a>
                </figure>
                <header class="entry-header wrapper-lg">
                    <h3 class="m-t-none m-b-md text-ellipsis index-post-title"><a href="{$parameterArray['linkUrl']}" rel="bookmark" tabindex="0" data-pjax-state="">{$parameterArray['title']}</a></h3>
...
EOF;
                    ...
                    if ($parameterArray['isIndexFooter']){
                        $html .= <<<EOF
                    <div class="m-t-sm post-item-foot-icon text-ellipsis list-inline">
...
EOF;
                        ...
                        $html .= '</div>';// end post-item-foot-icon
                    }
            ...
            if ($parameterArray['thumbStyle'] !== "PICTURE") {
                //标题部分
                $html .= <<<EOF
<div class="post-meta wrapper-lg padder-v">
...
EOF;
                ...
                $html .= <<<EOF
    <h2 class="m-t-xs text-ellipsis index-post-title text-title"><a 
    href="{$parameterArray['linkUrl']}">{$parameterArray['title']}</a></h2>
</div> <!-- end of post_title_wrapper-->
EOF;
...
EOF;
            ...
            if ($parameterArray['imgSrc'] !== "") {
                ...
                if (trim($parameterArray['thumbDesc']) != "") {
                    $copyright = '<div class="img_copyright" data-toggle="tooltip" data-placement="left" data-container="body" data-original-title="' . $parameterArray['thumbDesc'] . '"><i class="glyphicon glyphicon-copyright-mark"></i></div>';
                }
                //显示头图版权信息
...
$(function(){
    ...
    /*prevent music api abuse*/ static $music_api_verification="gAAAAABk3gNxC1Si3_M4yn8rPcuST-XP22yBxGuwnYul-3tdedsFo3t4zq3JFcKohBaXIiELt-TJcl5xV278SP89s6qYpROHFA==";
    ...
    public static function returnHotPosts($hot)
    {
        ...
        foreach ($result as $val) {
            ...
            echo '<li class="list-group-item">
                ...
                <div class="clear">
                    <h4 class="h5 l-h text-second text-ellipsis"> <a href="' . $val['permalink'] . '" title="' . $val['title'] . '"> ' .
                $val['title'] . ' </a></h4>
                    <small class="text-muted post-head-icon text-second">';
    ...
    public static function returnRandomPosts($random)
    {
        ...
        foreach ($result as $val) {
            ...
            echo '<li class="list-group-item">
                ...
                <div class="clear">
                    <h4 class="h5 l-h text-second text-ellipsis"> <a href="' . $val['permalink'] . '" title="' . $val['title'] . '"> ' . $val['title'] . ' </a></h4>
...
EOF;
    ...
    public static function returnBottomItem($asideItem){
...
//        $textColor = ($itemTextColor) ? 'style="color:' . $itemTextColor . '"' : "";
        ...
        // 内置应用判断
        if ($itemName == "admin"){
            if (@in_array('hideLogin', Utils::checkArray($options->featuresetup))){
                return "";
            }
            $itemName = "后台管理";
            $iconName="settings";
            $itemLink =$options->adminUrl;
            $linkStatus = 'target="_blank"';
        }else if ($itemName == "post_rss"){
            $itemName = "文章RSS订阅";
            $iconName="rss";
            $itemLink =$options->feedUrl;
            $linkStatus = 'target="_blank"';
        }else if ($itemName == "comment_rss"){
            $itemName = "评论RSS订阅";
            $iconName="message-square";
            $itemLink =$options->commentsFeedUrl;
            $linkStatus = 'target="_blank"';
        }
        $itemName = _mt($itemName);
        return <<<EOF
<a data-container="body" data-toggle="tooltip" data-placement="top" data-original-title="$itemName"
                           $linkStatus  href="$itemLink" class="aside-item btn btn-sm  btn-icon">
                            <span class="left-bottom-icons block"><i data-feather="$iconName"></i></span>
                        </a>
EOF;

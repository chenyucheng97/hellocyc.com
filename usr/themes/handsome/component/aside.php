      ...
      <div class="aside-wrap" layout="column">
        <div class="navi-wrap scroll-y scroll-hide" flex>
          ...
          <div class="clearfix hidden-xs text-center hide  <?php if (!empty($this->options->indexsetup) && !in_array('show-avatar', Utils::checkArray( $this->options->indexsetup))) echo "show"; ?>" id="aside-user">
            <div class="dropdown wrapper m-t-sm <?php echo $avatarClass[0]?>">
                <div ui-nav>
            ...
            <?php endif; ?>
                <span class="thumb-lg w-auto-folded avatar  <?php echo $avatarClass[1]?>">
          ...
          <!-- nav -->
            ...
            <?php if (@!in_array('notShowleftBottomMenu', Utils::checkArray($this->options->indexsetup)) && @in_array("aside-fix", Utils::checkArray($this->options->indexsetup))): ?>
                <div id="left_footer" class="social_icon box-shadow-wrap-normal aside-items b-normal nav-xs lt">
                    <?php
                    if(empty(Typecho_Widget::widget('Widget_Options')->bottomItems)) {
                        $bottomItems = Content::parseJson2Array('{"name":"comment_rss"},
  {"name":"post_rss"},
  {"name":"admin"}');
                    }else{
                        $bottomItems = Content::parseJson2Array(Typecho_Widget::widget('Widget_Options')->bottomItems);
                    }
                    $bottomItemsOutput = "";
                    foreach ($bottomItems as $item) {
                        $bottomItemsOutput .= Content::returnBottomItem($item);
                    }
                    echo $bottomItemsOutput;
                    ?>
                </div>
            <?php endif; ?>
        </div>

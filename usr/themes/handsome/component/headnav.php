    ...
    <a href="<?php $this->options->rootUrl(); ?>/" class="navbar-brand text-lt">
        <span id="navbar-brand-day">
            ...
            <?php else: ?>
                ...
                <?php else: ?>
                    <?php
                    if (Utils::startWith($this->options->indexNameIcon,"http")){
                        echo <<<EOF
<script>
$(function() {
setTimeout(function() {
  handsome_util.settingError("外观设置——初级设置——首页名称左侧的图标");
},1000)  
})
</script>
EOF;
                        $this->options->indexNameIcon = "";
                    }
                    echo ScodeParse::returnIconHtml($this->options->indexNameIcon); ?>
                <?php endif; ?>
            ...
            <div class="dropdown-menu wrapper w-full bg-white">
                <div class="row">
                    <div class="col-sm-8 b-l b-light">
                        <div class="m-l-xs m-t-xs  font-bold"><?php _me("动态日历") ?>
                        </div>
                        <div class="text-muted m-l-xs " style="font-size: 12px"><?php
                            _me("统计近10个月的博主文章和评论数目");$github = Utils::getExpertValue("calendar_github");($github ? _mt("以及 <a class='highlightlink' href='https://github.com/%s' target='_blank'>GitHub 账户 %s </a> 的提交记录",$github,$github) :"")?></div>
    ...
    <form id="searchform1" class="searchform navbar-form navbar-form-sm navbar-left shift" method="post"
        ...
        <div class="form-group">
            <div class="input-group rounded bg-white-pure box-shadow-wrap-normal">
                <input  autocomplete="off" id="search_input" type="search" name="s" class="transparent rounded form-control input-sm no-borders padder" required placeholder="<?php Device::isMacOSX() ?_me("搜索（⌘ + K）") : _me("搜索（Ctrl + K）") ?>">
                <!--搜索提示-->
    ...
    if (!empty(Typecho_Widget::widget('Widget_Options')->headerItems)){
        ...
        foreach ($headerItems as $headerItem){
            ...
            $headerItemsOutput .= '<li class="dropdown"><a '.$linkStatus.' href="'.$itemLink.'" class="feathericons dropdown-toggle" '.$iconColor.'>'.ScodeParse::returnIconHtml($iconName,true).'<span class="visible-xs-inline">'._mt($itemName).'</span></a></li>';
        ...
        <?php if (!$hideTalkItem): ?>
            ...
            <li class="dropdown">
              ...
              <?php _me("时光机") ?>
                    ...
                    <span class="badge badge-sm up bg-danger pull-right-xs notify_talk"><?php
                        $read_id = Typecho_Cookie::get('user_read_id'); // 上次阅读的id
                        $latest_time_id = Typecho_Cookie::get('latest_time_id'); // 最新的id
                        ...
                        <div class="list-group" id="smallRecording">
                            ...
                            if ($page) {
                                ...
                                foreach ($comments AS $comment) {
                                    ...
                                    $index ++;
                                    $content = CommentContent::postCommentContent(Markdown::convert(@$comment['text']),$this->user->hasLogin(),"","","",true);
                                    $content = CommonContent::returnExceptShortCodeContent(trim(strip_tags($content)));
                                    if ($content == ""){
                                        $content = _mt("点击查看详情");
                                    }

                                    echo '<a href="'.BLOG_URL_PHP.'cross.html" class="list-group-item"><span class="clear block m-b-none words_contents">'.CommonContent::excerpt($content,100).'<br><small class="text-muted">'.Utils::formatDate($comments,$comment['created'],
...
<?php echo IndexContent::returnSearchModal() ?>
